...
for i, z_var_name in enumerate(z_var_names):
    ...
    fig_name=f'EA_2D_combine_15state_{z_var_name}'.replace(' ','_').replace('-','_') 
    for i in range(len(exp_id)):
        ...
        for idx, T in enumerate(T_0):
            ...
            ax[r,c].plot(X, Y2, marker='x', label=label+'_15state', color=color, linestyle='--')
            ax2=ax[r,c].twinx()
        ...
        if 'efficiency' in z_var_name:
            ...
            if min(np.min(Y1s), np.min(Y2s))>0:
                ...
                ymin = max(np.max(Y1s), np.max(Y2s)) * 0.6 # set to the half of the max values
