import sys
import os

from matplotlib.pylab import f
# Add parent directory to sys.path for module imports
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
# Add 'dataPlot' subdirectory to sys.path for module imports
sys.path.append(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'dataPlot'))

# Import Path for filesystem operations
from pathlib import Path
import matplotlib.pyplot as plt
import numpy as np

from matplotlib import cm

import pandas as pd
simulation_path = Path(__file__).parent.parent / 'cad' / 'models' / 'simulation'
path_ = Path(__file__).parent.parent / 'cad' /  'data'
exp_id=['Nai','Ko']
subtitles2=[r'Intracellular ${Na}^+$ concentration (mM)', r'Extracellular ${K}^+$  concentration (mM)']
T_0=np.array([600,800,1000,1200])
z_var_names=['Thermodynamic efficiency', 'Chemical conversion efficiency', 
             'Electrical conversion efficiency', 'Chemical-Electrical ratio',
             'ATP Hydrolysis Power']
subtitles=[r'Thermodynamic efficiency $\eta$ (%)', r'Chemical conversion efficiency $\eta_c$ (%)', 
           r'Electrical conversion efficiency $\eta_e$ (%)', r'Chemical-Electrical ratio $\gamma$',
           'ATP Hydrolysis Power (pW)']
# Chemical conversion efficiency, Electrical conversion efficiency, Chemical-Electrical ratio
basefile_1='EA_report_task_Terkildsen_NaK_kinetic_modular_V_'
basefile_2='EA_report_task_NKE_BG_15_state_'
col_map = cm.get_cmap('viridis', len(T_0))
xname='Mean flow rate'
xlabel='Mean turnover rate (/s)'

for i, z_var_name in enumerate(z_var_names):
    ncols=2
    nrows=2
    fig, ax = plt.subplots(nrows=nrows, ncols=ncols, figsize=(6,6))
    fig.subplots_adjust(left=0.1, bottom=0.15, right=0.95, top=0.95, wspace=0.3, hspace=0.3)
    #add title
   # fig.suptitle(f'{subtitles[i]}', fontsize=16)
    fig_name=f'EA_2D_combine_15state_{z_var_name}_NaK'.replace(' ','_').replace('-','_') 
    for i_exp in range(len(exp_id)):
        filename_T600=Path(simulation_path /f'{basefile_1}{exp_id[i_exp]}_T600ms.csv').as_posix()
        filename_T800=Path(simulation_path /f'{basefile_1}{exp_id[i_exp]}_T800ms.csv').as_posix()
        filename_T1000=Path(simulation_path /f'{basefile_1}{exp_id[i_exp]}_T1000ms.csv').as_posix()
        filename_T1200=Path(simulation_path /f'{basefile_1}{exp_id[i_exp]}_T1200ms.csv').as_posix()
        efficiency_T600=pd.read_csv(filename_T600)[z_var_name].values
        efficiency_T800=pd.read_csv(filename_T800)[z_var_name].values
        efficiency_T1000=pd.read_csv(filename_T1000)[z_var_name].values
        efficiency_T1200=pd.read_csv(filename_T1200)[z_var_name].values
        efficiency_T600_x=pd.read_csv(filename_T600)[xname].values
        efficiency_T800_x=pd.read_csv(filename_T800)[xname].values
        efficiency_T1000_x=pd.read_csv(filename_T1000)[xname].values
        efficiency_T1200_x=pd.read_csv(filename_T1200)[xname].values
        filename_T1000_2=Path(simulation_path /f'{basefile_2}{exp_id[i_exp]}_T1000ms.csv').as_posix()
        filename_T800_2=Path(simulation_path /f'{basefile_2}{exp_id[i_exp]}_T800ms.csv').as_posix()
        filename_T600_2=Path(simulation_path /f'{basefile_2}{exp_id[i_exp]}_T600ms.csv').as_posix()
        filename_T1200_2=Path(simulation_path /f'{basefile_2}{exp_id[i_exp]}_T1200ms.csv').as_posix()
        efficiency_T1000_2=pd.read_csv(filename_T1000_2)[z_var_name].values
        efficiency_T800_2=pd.read_csv(filename_T800_2)[z_var_name].values
        efficiency_T600_2=pd.read_csv(filename_T600_2)[z_var_name].values
        efficiency_T1200_2=pd.read_csv(filename_T1200_2)[z_var_name].values
        efficiency_T600_x_2=pd.read_csv(filename_T600_2)[xname].values
        efficiency_T800_x_2=pd.read_csv(filename_T800_2)[xname].values
        efficiency_T1000_x_2=pd.read_csv(filename_T1000_2)[xname].values
        efficiency_T1200_x_2=pd.read_csv(filename_T1200_2)[xname].values
        x2=pd.read_csv(Path(simulation_path /f'{exp_id[i_exp]}.csv').as_posix())[exp_id[i_exp]].values
        for idx, T in enumerate(T_0):
            if T==600:
                Y1, Y2, label = efficiency_T600, efficiency_T600_2, 'T=600ms'
                X1=efficiency_T600_x
                X2=efficiency_T600_x_2
            elif T==800:
                Y1, Y2, label = efficiency_T800, efficiency_T800_2, 'T=800ms'
                X1=efficiency_T800_x
                X2=efficiency_T800_x_2
            elif T==1000:
                Y1, Y2, label = efficiency_T1000, efficiency_T1000_2, 'T=1000ms'
                X1=efficiency_T1000_x
                X2=efficiency_T1000_x_2
            else:
                Y1, Y2, label = efficiency_T1200, efficiency_T1200_2, 'T=1200ms'
                X1=efficiency_T1200_x
                X2=efficiency_T1200_x_2
            color = col_map(idx)  
            if 'Power' in z_var_name: # absolute values
                Y1 = -np.array(Y1)/1000 # pW
                Y2 = -np.array(Y2)/1000
            ax_r, ax_c = divmod(i_exp, ncols) 
            # the first row, xdata is flow rate
            ax[0,ax_c].plot(X1, Y1, marker='.', label=label+'_ss', color=color)
            ax[0,ax_c].plot(X2, Y2, marker='x', label=label+'_15state', color=color, linestyle='--')
            ax[0,ax_c].set_xlabel(f'{xlabel}', fontsize=10)
            ax[0,ax_c].set_ylabel(f'{subtitles[i]}', fontsize=10)
            ax[0,ax_c].grid(True, linestyle='--', alpha=0.5)
            # the second row, xdata is Nai or Ko
            X= x2
            ax[1,ax_c].plot(X, X1, marker='.', label=label+'_ss', color=color)
            ax[1,ax_c].plot(X, X2, marker='x', label=label+'_15state', color=color, linestyle='--')
            ax[1,ax_c].set_xlabel(f' {subtitles2[i_exp]}', fontsize=10)
            ax[1,ax_c].set_ylabel(f'{xlabel}', fontsize=10)
            ax[1,ax_c].grid(True, linestyle='--', alpha=0.5)
            # show legend only on the first subplot
    ax[0,0].legend(title='Cycle length', loc='upper right', fontsize='small')     

    #plt.tight_layout()
    save_fig = {'save_fig': True, 'fig_format': 'png', 'file_path': simulation_path.as_posix()+'/', 'filename': fig_name}
    plt.savefig(save_fig['file_path'] + save_fig['filename'] + '.' + save_fig['fig_format'])
    plt.show()

